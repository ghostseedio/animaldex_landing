...
export const LIFECYCLE_RPC: Record<"submit" | "approve" | "reject" | "archive", {
    rpc: string;
...
export function groupCampaignsByStatus<T extends {status: string}>(campaigns: T[]) {
    const groups = Object.fromEntries(CAMPAIGN_STATUSES.map((status) => [status, [] as T[]])) as Record<CampaignStatus, T[]>;
...
export function canReviseRules(status: string) {
    return ["draft", "submitted", "rejected", "approved", "scheduled", "live"].includes(status);
}
...
export function authorshipLabel(sponsorOrganizationId: string | null, presenterName: string | null) {
    if (sponsorOrganizationId) {
        return presenterName?.trim() ? `Sponsored · ${presenterName.trim()}` : "Sponsored";
    }
    return presenterName?.trim() ? `AnimalDex · ${presenterName.trim()}` : "AnimalDex-authored";
}
...
export function nonVenueCountryRestrictionBlocked(geoMode: string, hasVenue: boolean) {
...
export function applyVenueConstraints(input: CampaignDraftInput): CampaignDraftInput {
...
export function serializeVenueUpsert(campaignId: string, input: VenueDraftInput) {
...
export function serializeGeoCountries(campaignId: string, countryCodes: string[]) {
    return {
        p_campaign_id: campaignId,
...
export function serializeAchievementReward(campaignId: string, input: AchievementDraftInput) {
...
export function serializeRulesRevision(campaignId: string, officialRules: string, rewardTerms: string) {
    return {
        p_campaign_id: campaignId,
...
export function parseAdminRpcError(message: string) {
    ...
    if (text.includes("apple_disclaimer_required")) {
        return {code: "apple_disclaimer_required", message: "Official rules must include the Apple disclaimer."};
    }
    if (text.includes("reward_required")) {
        return {code: "reward_required", message: "An achievement reward is required before submit."};
    }
    ...
    if (text.includes("unsupported_reward_type")) {
        return {code: "unsupported_reward_type", message: "Phase 1 supports achievement rewards only."};
    }
    if (text.includes("invalid_lifecycle")) {
        return {code: "invalid_lifecycle", message: "That lifecycle action is not available in the current status."};
    }
    if (text.includes("campaign_not_editable")) {
        return {code: "campaign_not_editable", message: "This campaign can no longer be edited. Use a rules revision for copy changes, or archive and create a new campaign for eligibility changes."};
    }
    return {code: "rpc_error", message};
}

export function currentRulesVersion(versions: AdminRuleVersion[], campaignVersion: number) {
    return versions.find((row) => row.rulesVersion === campaignVersion) ?? null;
}

export function historicalRulesVersions(versions: AdminRuleVersion[], campaignVersion: number) {
...
export function toListItem(row: {
    ...
    return {
        ...
        objectiveType: isObjectiveType(row.objective_type) ? row.objective_type : "eligible_capture_count",
...
export const SUPPORTED_REWARD_TYPES = ["achievement"] as const;
