...
const draft = {
  ...
  description:
    "A controlled AnimalDex-authored test Challenge for unique indexed entries.",
  presenterName: null,
  ...
  rewardTerms:
    "Completing the Challenge grants a deterministic achievement only.",
  liveOnly: false,
...
test("campaign list groups every lifecycle status and never treats archived as live", () => {
  ...
  assert.equal(
    groups.live.some((row) => row.status === "archived"),
    false,
  );
});
...
test("draft serialization matches the deployed upsert RPC contract", () => {
  ...
  assert.equal(
    authorshipLabel(
      payload.p_sponsor_organization_id,
      payload.p_presenter_name,
    ),
    "AnimalDex-authored",
  );
});
...
test("discovery country UI is explicitly non-authoritative", () => {
  ...
  assert.equal(
    DISCOVERY_GEOGRAPHY_COPY.toLowerCase().includes("participant eligibility"),
    false,
  );
  const geo = serializeGeoCountries("11111111-1111-4111-8111-111111111111", [
    "id",
    "SG",
    "xx1",
  ]);
...
test("non-venue restricted campaigns surface authoritative_country_unavailable", () => {
  assert.equal(
    publishBlockedReason("allowlist", false),
    "authoritative_country_unavailable",
  );
  assert.equal(
    publishBlockedReason("denylist", false),
    "authoritative_country_unavailable",
  );
...
test("rules history keeps current and accepted versions distinguishable", () => {
  ...
  assert.deepEqual(
    historicalRulesVersions(versions, 2).map((row) => row.rulesVersion),
    [1],
  );
  assert.notEqual(
    currentRulesVersion(versions, 2)?.officialRules,
    historicalRulesVersions(versions, 2)[0].officialRules,
  );
});

test("unsupported rewards cannot be configured", () => {
  assert.equal(isUnsupportedRewardType("achievement"), false);
  assert.equal(isUnsupportedRewardType("cash_and_achievement"), false);
  [
    "credits",
    "cash_only",
    "voucher",
...
test("cash rewards calculate a fixed maximum sponsor liability", () => {
  const cash = {
    amountMinor: 500,
    currencyCode: "usd",
    maxRecipients: 500,
  ...
  assert.equal(cashRewardLiability(cash), 250000);
  assert.equal(cashCampaignTotal(cash), 300000);
  assert.deepEqual(
    serializeCashReward("11111111-1111-4111-8111-111111111111", cash),
    {
      p_campaign_id: "11111111-1111-4111-8111-111111111111",
      p_amount_minor: 500,
      p_currency_code: "USD",
      p_max_recipients: 500,
      p_platform_fee_minor: 50000,
    },
  );
});
...
test("lifecycle buttons map only to privileged admin RPCs", () => {
  assert.equal(
    lifecycleButtons("draft").submit?.rpc,
    "admin_submit_sponsored_campaign",
  );
  assert.equal(
    lifecycleButtons("submitted").approve?.rpc,
    "admin_review_sponsored_campaign",
  );
  ...
  assert.equal(
    lifecycleButtons("live").archive?.rpc,
    "admin_archive_sponsored_campaign",
  );
...
test("admin DTOs contain no participant PII, GPS evidence, or service-role secrets", () => {
  ...
  assert.deepEqual(containsForbiddenAdminDtoKeys(leaked).sort(), [
...
test("Bird template serializes the type tag the backend evaluator will match", () => {
  const payload = serializeCampaignUpsert({
    ...draft,
    requiredTypeTag: "bird",
  });
  assert.equal(payload.p_required_type_tag, "Bird");
  assert.equal(
    backendTypeTagMatches("Bird", payload.p_required_type_tag!),
    true,
  );
...
test("Venue Collector setting serializes the value the backend evaluator will match", () => {
  ...
  assert.equal(
    backendSettingMatches("Zoo", payload.p_required_setting_tag!),
    true,
  );
...
test("lowercase setting storage would publish but never qualify captures", () => {
  ...
  const trapped = serializeCampaignUpsert({
    ...draft,
    requiredSettingTag: "zoo",
  });
  assert.notEqual(trapped.p_required_setting_tag, "zoo");
  assert.equal(
    backendSettingMatches("Zoo", trapped.p_required_setting_tag!),
    true,
  );
});
