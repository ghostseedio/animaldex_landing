...
export const PREVIEW_PROGRESS_NOTICE =
  "Preview progress is simulated. This is not live campaign data.";
...
export type PreviewCampaign = {
  ...
  cashRemainingRecipients: number | null;
...
export function isSponsoredCampaign(input: {
  sponsorOrganizationId?: string | null;
}): boolean {
...
export function cardAuthorship(input: PreviewCampaign): {
  ...
  if (isSponsoredCampaign(input)) {
    const presentedBy =
      input.presenterName?.trim() || input.sponsorDisplayName?.trim() || null;
...
export function progressUnitLabel(
  type: string,
): "entries" | "captures" | "days" {
  if (type === "unique_indexed_entries") return "entries";
  if (type === "active_capture_days") return "days";
  return "captures";
}

export function cardObjectiveLine(
  input: Pick<
    PreviewCampaign,
    | "objectiveType"
    | "targetCount"
...
export function qualificationBullets(
  input: Pick<
    PreviewCampaign,
    | "objectiveType"
    | "liveOnly"
  ...
  if (input.requiredTypeTag?.trim()) {
    bullets.push(
      `Must qualify as ${typeTagDisplayLabel(input.requiredTypeTag)}`,
    );
  }
  if (input.requiredSettingTag?.trim()) {
    bullets.push(
      `Must satisfy the ${settingTagDisplayLabel(input.requiredSettingTag)} setting`,
    );
  }
...
export function rewardLabel(
  ...
  const achievement = title ? `${title} achievement` : "Achievement";
  if (cashAmountMinor && currencyCode) {
    return `${formatEarningsMinor(cashAmountMinor, currencyCode)} + ${achievement}`;
  }
  return achievement;
}

export function simulatedJoinedProgress(targetCount: number): number {
  const target = Number.isFinite(targetCount)
...
export function toPreviewCampaign(input: {
  ...
  return {
    ...
    cashAmountMinor: input.cashAmountMinor ?? null,
    cashCurrencyCode: input.cashCurrencyCode?.trim().toUpperCase() || null,
    cashRemainingRecipients: input.cashRemainingRecipients ?? null,
    thumbnailUrl: input.thumbnailUrl?.trim() || null,
    thumbnailAltText: input.thumbnailAltText?.trim() || null,
