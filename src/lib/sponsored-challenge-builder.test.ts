...
function readyDraft(): CampaignDraftInput {
  return {
    ...
    publicSummary:
      "Capture 15 different qualifying AnimalDex entries at BXSea.",
    description:
...
test("templates persist canonical backend tags, not display-only casing", () => {
  ...
  const venuePayload = serializeCampaignUpsert({
    ...venue,
    slug: "venue",
    title: "Venue",
    publicSummary: "s",
  ...
  const birdPayload = serializeCampaignUpsert({
    ...bird,
    slug: "bird",
    title: "Bird",
    publicSummary: "s",
  ...
  assert.equal(
    backendSettingMatches("Zoo", venuePayload.p_required_setting_tag!),
    true,
  );
  assert.equal(
    backendTypeTagMatches("Bird", birdPayload.p_required_type_tag!),
    true,
  );
...
test("Photography Quality defaults", () => {
  const draft = applyTemplateDefaults(
    "photography_quality",
    blankDraft(now),
    now,
  );
...
test("slug is generated from title and stays put after a manual edit", () => {
  assert.equal(
    slugifyTitle("BXSea Ocean Collector 2026"),
    "bxsea-ocean-collector-2026",
  );
  ...
  const draft = {
    ...applyTemplateDefaults("blank", blankDraft(now), now),
    title: "BXSea Ocean Collector 2026",
  };
  ...
  const current = currentGeneratedCopy(
    source({
      draft: {
        ...draft,
        slug: "custom-slug",
...
test("AnimalDex-authored presenter autofill does not invent a sponsor", () => {
  ...
  const generated = generateBuilderCopy(
    source({
      draft: {
        ...applyTemplateDefaults("bird_challenge", blankDraft(now), now),
  ...
  assert.equal(
    generated.presenterName.toLowerCase().includes("sponsored"),
    false,
  );
});
...
test("summary generation stays plain and factual", () => {
  ...
  assert.equal(
    generatePublicSummary(bird),
    "Capture 20 different qualifying bird entries during the Challenge.",
  );
  const grade = source({
  ...
  assert.equal(
    generatePublicSummary(grade),
    "Complete 5 qualifying captures graded 8 or higher.",
  );
  const days = source({
  ...
  assert.equal(
    generatePublicSummary(days),
    "Make qualifying AnimalDex captures on 5 different days.",
  );
});
...
test("official rules reflect configured backend fields only", () => {
  const rules = generateOfficialRules(
    source({
      venueName: "BXSea",
      draft: {
        ...applyTemplateDefaults("venue_collector", blankDraft(now), now),
  ...
  assert.match(
    rules,
    /collect 15 different qualifying indexed AnimalDex entries/,
  );
  ...
  assert.match(
    rules,
    /configured BXSea venue using trusted device-location evidence/,
  );
  ...
  assert.match(
    rules,
    new RegExp(APPLE_DISCLAIMER.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
  );
...
test("grade and type constraints appear in rules only when configured", () => {
  const bird = generateOfficialRules(
    source({
      templateId: "bird_challenge",
      draft: applyTemplateDefaults("bird_challenge", blankDraft(now), now),
    }),
  ...
  const grade = generateOfficialRules(
    source({
      templateId: "photography_quality",
      draft: applyTemplateDefaults("photography_quality", blankDraft(now), now),
    }),
  ...
  assert.equal(
    grade.includes("Each indexed AnimalDex entry counts once"),
    false,
  );
});

test("imports language follows the configured backend flag", () => {
  const blocked = generateOfficialRules(
    source({
      draft: {
        ...applyTemplateDefaults("blank", blankDraft(now), now),
  ...
  const allowed = generateOfficialRules(
    source({
      templateId: "blank",
      draft: {
        ...applyTemplateDefaults("blank", blankDraft(now), now),
  ...
  assert.equal(
    allowed.includes("Imported or external captures do not qualify"),
    false,
  );
});
...
test("reward terms use the achievement title and stay Phase 1 only", () => {
  ...
  assert.match(
    terms,
    /No cash, Credits, sweepstakes, random drawing, or prize of chance/,
  );
...
test("manual fields are not rewritten when source fields change", () => {
  ...
  const current = currentGeneratedCopy(
    source({
      draft: {
        ...applyTemplateDefaults("venue_collector", blankDraft(now), now),
        publicSummary: "Operator written summary.",
      },
    }),
  );
...
test("sponsored card disclosure and AnimalDex-authored card stay distinct", () => {
  ...
  assert.equal(
    cardObjectiveLine(sponsored),
    "Capture 15 different AnimalDex entries",
  );
  assert.equal(
    formatPreviewDateRange(sponsored.startsAt, sponsored.endsAt),
    "Sep 1 – Sep 30",
  );
  assert.equal(
    rewardLabel(sponsored.rewardTitle),
    "BXSea Ocean Collector achievement",
  );
  assert.ok(
    qualificationBullets(sponsored).includes("Must be recorded at BXSea"),
  );
  assert.ok(
    qualificationBullets(sponsored).includes(
      "Each indexed AnimalDex entry counts once",
    ),
  );
  assert.ok(
    qualificationBullets(sponsored).includes(
      "Imported captures do not qualify",
    ),
  );
  ...
  assert.ok(
    qualificationBullets(tagged).includes("Must satisfy the Zoo setting"),
  );
...
test("preview displays deterministic cash plus achievement without forbidden prize mechanics", () => {
  const label = rewardLabel("BXSea Ocean Collector", 500, "USD");
    assert.equal(label, "$5.00 + BXSea Ocean Collector achievement");
  assert.equal(previewShowsUnsupportedReward(label), false);
  assert.equal(
    previewShowsUnsupportedReward("One random drawing wins a prize pool"),
    true,
  );
});
...
test("missing reward blocks readiness", () => {
  ...
  assert.match(
    result.fieldWarnings.reward ?? "",
    /No achievement reward configured/,
  );
});

test("cash campaign readiness blocks until full funding is confirmed", () => {
  const base = {
    draft: readyDraft(),
    templateId: "venue_collector" as const,
    venueName: "BXSea",
    venueLat: "-6.1",
    venueLng: "106.8",
    venueRadius: "400",
    achievementSlug: "bxsea",
    achievementTitle: "BXSea",
    achievementDetail: "Complete BXSea",
    cashEnabled: true,
    hasPersistedDraft: true,
    persistedRulesVersion: 1,
    hasHistoricalRules: false,
  ...
  assert.ok(
    evaluateReadiness({ ...base, cashFundingStatus: "draft" }).blocking.some(
      (check) => check.id === "cash_funding",
    ),
  );
  assert.equal(
    evaluateReadiness({
      ...base,
      cashFundingStatus: "confirmed",
    }).blocking.some((check) => check.id === "cash_funding"),
    false,
  );
});
...
test("venue country targeting is a discovery-only warning", () => {
  ...
  assert.equal(
    result.blocking.some((check) => check.id === "geo_authority"),
    false,
  );
