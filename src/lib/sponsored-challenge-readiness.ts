...
function venueConfigured(input: ReadinessInput): boolean {
    return present(input.venueName) || present(input.venueLat) || present(input.venueLng) || input.draft.hasVenue;
}
...
export function evaluateReadiness(input: ReadinessInput): ReadinessResult {
    ...
    const importsAllowed = needsVenue ? false : input.draft.externalImportsAllowed;

    const required: Array<[string, string, boolean, string?]> = [
        ["title", "Title present", present(input.draft.title)],
        ["slug", "Valid slug", isValidCampaignSlug(input.draft.slug), "Use a lowercase hyphenated slug."],
        ...
        ["objective", "Objective present", isObjectiveType(input.draft.objectiveType)],
        ["target", "Target greater than 0", Number.isInteger(input.draft.targetCount) && input.draft.targetCount > 0],
        ...
        ["achievement", "Achievement reward configured", achievementConfigured, "Phase 1 requires an achievement reward."]
    ];
    ...
    if (!achievementConfigured) fieldWarnings.reward = "No achievement reward configured.";
    ...
    if (needsVenue) {
        checks.push({
            ...
            detail: present(input.venueName) ? undefined : "Venue campaign has no venue saved."
        });
        checks.push({
            id: "venue_coords",
            label: "Venue latitude and longitude",
            severity: parseCoordinate(input.venueLat) != null && parseCoordinate(input.venueLng) != null ? "ready" : "blocking"
        });
        checks.push({
            id: "venue_radius",
            label: "Valid validation radius",
            severity: radiusValid ? "ready" : "blocking",
            detail: radiusValid ? undefined : `Validation radius must be ${VENUE_VALIDATION_RADIUS_MIN}–${VENUE_VALIDATION_RADIUS_MAX} m.`
        });
        checks.push({
            id: "venue_live",
        ...
        checks.push({
            ...
            detail: importsAllowed ? "Live venue campaigns cannot allow imports." : undefined
        });
        if (!present(input.venueName) || !hasVenueComplete) {
            fieldWarnings.venue = "Venue campaign has no venue saved.";
        }
        if (importsAllowed) fieldWarnings.imports = "Live venue campaigns cannot allow imports.";
    ...
    if (geoBlocked) {
        ...
        fieldWarnings.geo = "This non-venue country restriction cannot be enforced.";
    } else if (input.draft.geoMode !== "unrestricted" || input.discoveryCountries.length > 0) {
        checks.push({
            id: "geo_discovery",
    ...
    if (sponsored) {
        checks.push({
            ...
            severity: present(input.draft.sponsorOrganizationId) ? "ready" : "blocking"
        });
        checks.push({
            id: "presenter",
        ...
        if (!present(input.draft.presenterName)) {
            fieldWarnings.sponsor = "Sponsored campaigns need a presenter display name.";
        }
    } else {
        checks.push({
            id: "authorship",
    ...
    const hasDisclaimer = input.draft.officialRules.includes(APPLE_DISCLAIMER)
    ...
    checks.push({
        ...
        detail: hasDisclaimer ? undefined : "Official rules must include the Apple disclaimer."
    });
    ...
    if (input.hasPersistedDraft) {
        checks.push({
            ...
            severity: input.persistedRulesVersion != null && input.persistedRulesVersion > 0 ? "ready" : "blocking",
            detail: input.persistedRulesVersion ? undefined : "Save the campaign to persist rules version 1."
        });
    } else {
        checks.push({
            id: "rules_version",
