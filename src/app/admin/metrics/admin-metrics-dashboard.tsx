...
function GrowthCommandCenter({growth, month, setMonth, reload}: {growth: GrowthData | null; month: string; setMonth: (month: string) => void; reload: () => Promise<void>}) {
    ...
    async function createPlan(copy: boolean) {
        ...
        await fetch("/api/admin/growth", {method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({action: "save-plan", month, targets: source?.targets ?? blankTargets, weeklyTargets: source?.weeklyTargets ?? [], weeklyActionPlans: source?.weeklyActionPlans ?? []})});
        await reload();
    }
    ...
    async function savePlan(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            setPlanError(caught instanceof Error ? caught.message : "Invalid plan JSON");
        } finally {
            setSaving(false);
        }
    }

    return <section className="mt-6 rounded-2xl border border-line-300 bg-surface-900 p-4 sm:p-6">
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
            <div><p className="text-xs font-black uppercase tracking-[.18em] text-primary-200">Month plan</p><h2 className="mt-2 font-display text-3xl text-white">{monthLabel(month)} Growth Plan</h2><p className="mt-1 text-xs text-ink-500">Calendar timezone: {growth?.timezone ?? "Asia/Jakarta"}</p></div>
            <div className="flex flex-wrap items-center gap-2 text-sm font-bold"><button className="rounded-xl border border-line-300 px-3 py-2 text-ink-300" onClick={() => setMonth(shiftMonth(month, -1))}>← {monthLabel(shiftMonth(month, -1))}</button><span className="rounded-xl bg-primary-500 px-3 py-2 text-canvas-950">{monthLabel(month)}</span><button className="rounded-xl border border-line-300 px-3 py-2 text-ink-300" onClick={() => setMonth(shiftMonth(month, 1))}>{monthLabel(shiftMonth(month, 1))} →</button>{plan ? <button className="rounded-xl border border-line-300 px-3 py-2 text-white" onClick={() => setEditingPlan(true)}>Edit plan</button> : null}</div>
        </div>
        {!plan ? <div className="mt-6 rounded-xl border border-line-300 bg-canvas-900 p-5"><h3 className="font-display text-2xl text-white">Create {monthLabel(month)} Growth Plan</h3><div className="mt-4 flex flex-wrap gap-3"><button onClick={() => createPlan(true)} className="rounded-xl bg-primary-500 px-4 py-2 text-sm font-black text-canvas-950">Copy previous targets</button><button onClick={() => createPlan(false)} className="rounded-xl border border-line-300 px-4 py-2 text-sm font-black text-white">Start blank</button></div></div> : <>
            <PlanResult result={growth?.monthResult} previous={growth?.previousMonthResult} />
            <div className={`mt-5 rounded-xl border p-4 ${statusClass(statusForMetric(usersActual, usersExpected))}`}><p className="text-xs font-black uppercase tracking-[.15em]">{userDelta >= 0 ? "You are on target" : "You are behind target"}</p><p className="mt-2 text-sm text-ink-100">{format(usersActual)} users so far. You would expect {format(usersExpected)} by today, so you are {format(Math.abs(userDelta))} users {userDelta >= 0 ? "ahead" : "behind"}. {userDelta < 0 ? `You need ${format(required)} users/day for the rest of ${monthLabel(month)}.` : ""}</p></div>
            <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{(Object.keys(growthMeta) as GrowthMetricKey[]).map((key) => { const target = plan.targets[key]; const actual = growth?.actuals?.[key] ?? 0; const expected = key === "activePro" ? target : expectedByDay(target, currentDay || totalDays, totalDays); const status = statusForMetric(actual, expected, {lowerIsBudget: growthMeta[key].budget}); const percent = target ? Math.min(999, Math.round(actual / target * 100)) : 0; const delta = growthMeta[key].budget ? expected - actual : actual - expected; return <div key={key} className="rounded-xl border border-line-300 bg-canvas-900 p-4"><div className="flex items-center justify-between gap-2"><p className="text-xs font-black uppercase tracking-[.14em] text-ink-400">{growthMeta[key].label}</p><span className="rounded-full border border-line-300 px-2 py-1 text-[10px] font-black text-ink-300">{growthMeta[key].kind}</span></div><p className="mt-3 font-display text-3xl text-white">{format(actual, growthMeta[key].money)} / {format(target, growthMeta[key].money)}</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/[.07]"><div className="h-full rounded-full bg-primary-400" style={{width: `${Math.min(100, percent)}%`}} /></div><div className="mt-2 flex items-center justify-between text-xs text-ink-400"><span>{percent}% complete</span><span>Expected: {format(expected, growthMeta[key].money)}</span></div><p className="mt-2 text-xs text-ink-500">{growthMeta[key].budget ? (delta >= 0 ? `${format(delta, true)} budget left to today pace` : `${format(Math.abs(delta), true)} over pace`) : (delta >= 0 ? `+${format(delta)} ahead` : `${format(delta)} behind`)}</p><span className={`mt-3 inline-flex rounded-full border px-2.5 py-1 text-[10px] font-black ${statusClass(status)}`}>{statusLabel(status)}</span></div>;})}</div>
            <div className="mt-6 rounded-xl border border-line-300 bg-canvas-900 p-4"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h3 className="font-display text-2xl text-white">{growthMeta[chartMetric].label} - Target vs Actual</h3><p className="text-xs text-ink-500">Cumulative month trajectory</p></div><div className="grid grid-cols-2 gap-1 rounded-xl border border-line-300 bg-surface-900 p-1 sm:flex">{(["users", "captures", "socialViews", "searchClicks"] as GrowthChartMetric[]).map((key) => <button key={key} onClick={() => setChartMetric(key)} className={`rounded-lg px-3 py-2 text-xs font-black ${chartMetric === key ? "bg-primary-500 text-canvas-950" : "text-ink-400"}`}>{growthMeta[key].label}</button>)}</div></div><MiniChart rows={growth?.daily ?? []} metric={chartMetric} target={plan.targets[chartMetric]} /></div>
            <div className="mt-6"><h3 className="font-display text-2xl text-white">Weekly progress</h3><div className="mt-3 grid gap-3 xl:grid-cols-5">{plan.weeklyTargets.map((week) => { const weekActuals = growth?.weeklyActuals?.find((row) => row.label === week.label)?.actuals; const actual = weekActuals?.users ?? 0; const future = currentDay > 0 && week.startDay > currentDay; const status = statusForMetric(actual, week.targets.users, {future}); const current = currentDay >= week.startDay && currentDay <= week.endDay; return <div key={week.label} className={`rounded-xl border p-4 ${current ? "border-primary-300 bg-primary-500/[.08]" : "border-line-300 bg-canvas-900"}`}><p className="text-xs font-black uppercase tracking-[.14em] text-ink-400">{week.label} · {week.startDay}-{week.endDay}</p><p className="mt-2 text-sm text-white">Users {format(actual)} / {format(week.targets.users)}</p><p className="mt-1 text-xs text-ink-400">Captures {format(weekActuals?.captures ?? 0)} / {format(week.targets.captures)}</p><p className="mt-1 text-xs text-ink-400">Views {format(weekActuals?.socialViews ?? 0)} / {format(week.targets.socialViews)}</p><span className={`mt-3 inline-flex rounded-full border px-2 py-1 text-[10px] font-black ${statusClass(status)}`}>{statusLabel(status)}</span></div>;})}</div></div>
            <details className="mt-6 rounded-xl border border-line-300 bg-canvas-900 p-4" open><summary className="cursor-pointer font-display text-2xl text-white">This week plan</summary><div className="mt-3"><p className="text-sm font-black text-primary-100">{thisWeek?.label}</p><ul className="mt-3 grid gap-2 text-sm text-ink-300 sm:grid-cols-2">{thisWeek?.items.map((item) => <li key={item} className="rounded-lg border border-line-300 bg-surface-900 px-3 py-2">{item}</li>)}</ul></div></details>
            <div className="mt-6"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h3 className="font-display text-2xl text-white">Daily pace</h3><p className="text-xs text-ink-500">Manual entries are daily values, not cumulative totals.</p></div><button onClick={() => setEditingDate((growth?.daily ?? []).find((row) => row.date === growth?.today) ?? growth?.daily?.[0] ?? null)} className="rounded-xl bg-primary-500 px-4 py-2 text-sm font-black text-canvas-950">Update marketing numbers</button></div><div className="mt-3 overflow-x-auto rounded-xl border border-line-300"><table className="min-w-[860px] w-full text-left text-sm"><thead className="bg-canvas-900 text-xs uppercase tracking-[.14em] text-ink-500"><tr>{["Date", "Users", "Captures", "Social views", "Google clicks", "Ad spend", "Status", ""].map((head) => <th key={head} className="px-3 py-3">{head}</th>)}</tr></thead><tbody>{(growth?.daily ?? []).map((row) => { const future = growth?.today ? row.date > growth.today : false; const status = statusForMetric(row.users, expectedByDay(plan.targets.users, 1, totalDays), {future}); const today = row.date === growth?.today; return <tr key={row.date} className={today ? "bg-primary-500/[.08]" : "odd:bg-white/[.02]"}><td className="px-3 py-2 font-bold text-white">{new Intl.DateTimeFormat("en", {month: "short", day: "numeric", timeZone: "UTC"}).format(new Date(`${row.date}T00:00:00Z`))}</td><td className="px-3 py-2">{format(row.users)} <span className="text-[10px] text-primary-100">AUTO</span></td><td className="px-3 py-2">{format(row.captures)} <span className="text-[10px] text-primary-100">AUTO</span></td><td className="px-3 py-2">{format(row.marketing.socialViews)} <span className="text-[10px] text-amber-200">MANUAL</span></td><td className="px-3 py-2">{format(row.marketing.searchClicks)} <span className="text-[10px] text-amber-200">MANUAL</span></td><td className="px-3 py-2">{format(row.marketing.adSpend, true)}</td><td className="px-3 py-2"><span className={`rounded-full border px-2 py-1 text-[10px] font-black ${statusClass(status)}`}>{today ? "TODAY" : statusLabel(status)}</span></td><td className="px-3 py-2"><button onClick={() => setEditingDate(row)} className="text-xs font-black text-primary-100">Edit</button></td></tr>;})}</tbody></table></div></div>
        </>}
...
export default function AdminMetricsDashboard() {
    ...
    return <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(27,196,81,.1),transparent_28%)] p-4 text-ink-100 sm:p-7"><div className="mx-auto max-w-[96rem]"><header className="flex flex-col justify-between gap-4 border-b border-line-300 pb-6 sm:flex-row sm:items-end"><div><Link href="/admin" className="text-sm text-ink-400 hover:text-white">← Admin</Link><h1 className="mt-3 font-display text-4xl text-white sm:text-5xl">Growth metrics</h1><p className="mt-2 text-sm text-ink-400">{rangeLabel} · production purchases separated from sandbox</p><Link href="/admin/users" className="mt-3 inline-flex text-sm font-bold text-primary-100 hover:text-primary-200">View users, buyers & LTV →</Link></div><div className="grid w-full grid-cols-3 rounded-xl border border-line-300 bg-surface-900 p-1 sm:flex sm:w-auto">{(["day", "week", "month"] as Period[]).map((item) => <button key={item} onClick={() => {setPeriod(item); void loadMetrics(item);}} className={`rounded-lg px-4 py-2 text-xs font-black capitalize ${period === item ? "bg-primary-500 text-canvas-950" : "text-ink-400"}`}>{item}</button>)}</div></header>{error && <div className="mt-5 rounded-xl border border-red-400/20 bg-red-500/10 p-3 text-sm text-red-200">{error}</div>}{loading && !data ? <div className="py-20 text-center text-ink-400">Loading growth data...</div> : data && <><GrowthCommandCenter growth={growth} month={growthMonth} setMonth={setGrowthMonth} reload={() => loadGrowth(growthMonth)} /><section className="mt-8 border-t border-line-300 pt-7"><p className="text-xs font-black uppercase tracking-[.18em] text-primary-200">Product growth</p><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{(Object.keys(metricMeta) as ChartMetric[]).map((key) => { const item = data.kpis[key]; return <button key={key} onClick={() => setMetric(key)} className={`rounded-2xl border p-5 text-left transition ${metric === key ? "border-primary-300 bg-primary-500/[.08]" : "border-line-300 bg-surface-900 hover:border-line-200"}`}><div className="flex items-center justify-between"><p className="text-xs font-black uppercase tracking-[.15em] text-ink-400">{metricMeta[key].label}</p><span className={`rounded-full px-2 py-1 text-[10px] font-black ${item.change >= 0 ? "bg-primary-500/15 text-primary-100" : "bg-red-500/15 text-red-200"}`}>{item.change >= 0 ? "+" : ""}{item.change}%</span></div><p className="mt-3 font-display text-4xl text-white">{format(item.value)}</p><p className="mt-1 text-xs text-ink-500">{metricMeta[key].description}</p></button>;})}</div><section className="mt-5 rounded-2xl border border-line-300 bg-surface-900 p-4 sm:p-6"><div className="flex items-center justify-between"><div><h2 className="font-display text-2xl text-white">{metricMeta[metric].label}</h2><p className="text-xs text-ink-500">{rangeLabel}</p></div><span className="h-3 w-3 rounded-full" style={{background: metricMeta[metric].color}} /></div><div className="mt-4"><TrendChart rows={data.series} metric={metric} period={period} /></div></section></section><section className="mt-8 border-t border-line-300 pt-7"><p className="text-xs font-black uppercase tracking-[.18em] text-primary-200">Users / monetisation</p><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{Object.entries(data.totals).map(([key, value]) => <div key={key} className="rounded-2xl border border-line-300 bg-canvas-900 p-4"><p className="text-xs capitalize text-ink-500">{key.replace(/([A-Z])/g, " $1")}</p><p className="mt-2 font-display text-2xl text-white">{format(value)}</p></div>)}</div></section>{data.signIn && <section className="mt-8 border-t border-line-300 pt-7"><p className="text-xs font-black uppercase tracking-[.18em] text-primary-200">Accounts</p><h2 className="mt-2 font-display text-3xl text-white">How people sign in</h2><p className="mt-2 max-w-3xl text-sm text-ink-400">{format(data.signIn.total)} accounts. A provider is not a platform, so device evidence is counted separately.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{Object.entries(data.signIn.providers).sort((left, right) => right[1] - left[1]).map(([provider, count]) => <div key={provider} className="rounded-2xl border border-line-300 bg-canvas-900 p-4"><p className="text-xs capitalize text-ink-500">{provider}</p><p className="mt-2 font-display text-2xl text-white">{format(count)}</p></div>)}</div><div className="mt-3 rounded-2xl border border-line-300 bg-canvas-900 p-4"><p className="text-xs text-ink-500">Confirmed Apple devices</p><p className="mt-2 font-display text-2xl text-white">{format(data.signIn.appleDeviceSignals)}</p><p className="mt-2 text-[11px] leading-4 text-ink-500">{data.signIn.note}</p></div></section>}<section className="mt-8 border-t border-line-300 pt-7"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-xs font-black uppercase tracking-[.18em] text-primary-200">Discover activity</p><h2 className="mt-2 font-display text-3xl text-white">Post types</h2><p className="mt-2 text-sm text-ink-400">What users are sharing across the community timeline · {format(data.postActivity.total)} total in this period</p></div><span className="w-fit rounded-full border border-line-300 bg-surface-900 px-3 py-1.5 text-xs font-bold text-ink-300">{rangeLabel}</span></div><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{(Object.keys(postTypeMeta) as PostType[]).sort((left, right) => data.postActivity.types[right].value - data.postActivity.types[left].value).map((key, index) => { const item = data.postActivity.types[key]; const share = data.postActivity.total ? Math.round(item.value / data.postActivity.total * 100) : 0; return <div key={key} className="rounded-2xl border border-line-300 bg-surface-900 p-4"><div className="flex items-center justify-between gap-2"><div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full" style={{background: postTypeMeta[key].color}} /><p className="text-xs font-black text-white">{postTypeMeta[key].shortLabel}</p></div><span className="text-[10px] font-bold text-ink-500">#{index + 1}</span></div><div className="mt-4 flex items-end justify-between gap-2"><p className="font-display text-3xl text-white">{format(item.value)}</p><span className={`rounded-full px-2 py-1 text-[10px] font-black ${item.change >= 0 ? "bg-primary-500/15 text-primary-100" : "bg-red-500/15 text-red-200"}`}>{item.change >= 0 ? "+" : ""}{item.change}%</span></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/[.06]"><div className="h-full rounded-full" style={{width: `${share}%`, background: postTypeMeta[key].color}} /></div><p className="mt-2 text-[11px] text-ink-500">{share}% of activity · {postTypeMeta[key].description}</p></div>;})}</div><div className="mt-5 rounded-2xl border border-line-300 bg-surface-900 p-4 sm:p-6"><div className="flex flex-wrap gap-x-4 gap-y-2">{(Object.keys(postTypeMeta) as PostType[]).map((key) => <span key={key} className="flex items-center gap-1.5 text-[11px] font-bold text-ink-300"><span className="h-2 w-2 rounded-sm" style={{background: postTypeMeta[key].color}} />{postTypeMeta[key].shortLabel}</span>)}</div><div className="mt-4"><PostTypeChart rows={data.postActivity.series} period={period} /></div></div></section></> }<section className="mt-8 border-t border-line-300 pt-7"><div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-xs font-black uppercase tracking-[.18em] text-primary-200">Audience</p><h2 className="mt-2 font-display text-3xl text-white">Social channels</h2></div><button onClick={syncSocial} disabled={socialLoading} className="w-full rounded-xl border border-line-300 px-4 py-2 text-xs font-black text-white disabled:opacity-50 sm:w-auto">{socialLoading ? "Syncing..." : "Sync now"}</button></div><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{social.map((item) => <div key={item.platform} className="rounded-2xl border border-line-300 bg-surface-900 p-4"><div className="flex items-center justify-between"><h3 className="font-bold text-white">{item.platform}</h3><span className={`h-2.5 w-2.5 rounded-full ${item.configured && !item.error ? "bg-primary-300" : "bg-amber-300"}`} /></div>{item.configured ? <><div className="mt-4 flex items-end justify-between gap-2"><div><p className="font-display text-3xl text-white">{format(item.followers)}</p><p className="text-xs text-ink-500">followers</p></div>{item.followerChange != null && <span className={`rounded-full px-2 py-1 text-[10px] font-black ${item.followerChange >= 0 ? "bg-primary-500/15 text-primary-100" : "bg-red-500/15 text-red-200"}`}>{item.followerChange >= 0 ? "+" : ""}{format(item.followerChange)}</span>}</div><div className="mt-4 flex gap-4 text-xs text-ink-300"><span>{format(item.views)} views/likes{item.viewChange != null ? ` (${item.viewChange >= 0 ? "+" : ""}${format(item.viewChange)})` : ""}</span><span>{format(item.posts)} posts</span></div>{item.error && <p className="mt-3 text-xs text-red-300">{item.error}</p>}</> : <p className="mt-4 text-sm leading-6 text-ink-400">Add this platform API credentials to enable syncing.</p>}</div>)}</div><p className="mt-3 text-xs text-ink-500">Social platforms only expose metrics through approved APIs. Snapshot history begins when credentials are configured and Sync now is used.</p></section></div></main>;
}
