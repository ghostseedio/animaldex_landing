...
const apps: Array<{name: string; values: Support[]}> = [
    {name: "AnimalDex", values: ["yes", "yes", "yes", "no", "no", "yes", "yes", "no", "yes", "yes", "yes", "yes", "yes"]},
    {name: "iNaturalist", values: ["yes", "yes", "yes", "yes", "limited", "yes", "yes", "yes", "yes", "no", "no", "yes", "no"]},
    {name: "Seek", values: ["yes", "yes", "yes", "yes", "yes", "yes", "yes", "no", "yes", "yes", "no", "yes", "no"]},
    {name: "Merlin Bird ID", values: ["no", "yes", "no", "no", "yes", "yes", "yes", "no", "yes", "limited", "no", "yes", "no"]},
    {name: "Google Lens", values: ["limited", "yes", "yes", "yes", "no", "no", "yes", "no", "limited", "no", "no", "yes", "no"]},
    {name: "PictureThis", values: ["no", "limited", "limited", "yes", "limited", "yes", "yes", "no", "yes", "no", "no", "limited", "yes"]}
];

const criteria = ["All animals", "Birds", "Insects", "Plants", "Offline", "Collection", "AI ID", "Community verification", "Education", "Progression", "Personal growth", "Free", "Premium"];
...
function Mark({value}: {value: Support}) {
    if (value === "yes") return <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-emerald-500/15 font-black text-emerald-300" aria-label="Supported">✓</span>;
    if (value === "limited") return <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-amber-500/15 font-black text-amber-300" aria-label="Limited">◐</span>;
    return <span className="text-ink-500" aria-label="Not supported">—</span>;
}

export function AppComparisonTable() {
    return (
        <div className="space-y-3">
            <p className="text-sm text-ink-300 md:hidden">Swipe sideways to compare every feature.</p>
            <div className="overflow-x-auto rounded-[2rem] border border-line-300 bg-surface-900/75 shadow-2xl">
                <table className="w-full min-w-[76rem] border-collapse text-center text-sm">
                    <thead><tr className="border-b border-line-300 bg-surface-800/95"><th className="sticky left-0 z-20 min-w-44 bg-surface-800 px-5 py-5 text-left font-display text-lg text-white">App</th>{criteria.map((criterion) => <th key={criterion} className="min-w-24 px-3 py-5 text-xs font-bold uppercase tracking-[0.08em] text-ink-200">{criterion}</th>)}</tr></thead>
