...
export async function generateMetadata(): Promise<Metadata> {
    ...
    const title = messages.challenges?.metaTitle || "AnimalDex Challenges";
    const description = messages.challenges?.metaDescription || messages.meta?.description || "";
    ...
    return {
        ...
        alternates: {
            canonical: getLocalePath(locale, "/challenges"),
            languages: localeConfig.locales.reduce((acc, localeItem) => {
                acc[localeItem] = `/${localeItem}/challenges`;
                return acc;
            }, {
                "x-default": `/${localeConfig.defaultLocale}/challenges`
        ...
        openGraph: {
            ...
            description,
            url: getLocalePath(locale, "/challenges"),
            images: [
                {
...
export default async function ChallengesIndexPage() {
    ...
    const locale = await getLocale();
    const pageUrl = getAbsoluteUrl(locale, "/challenges");
    ...
    const itemListSchema = {
        "@context": "https://schema.org",
        "@type": "ItemList",
        itemListElement: challengeEntries.map((entry, index) => ({
            "@type": "ListItem",
            position: index + 1,
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {challengeEntries.map((entry) => {
                    ...
                    return (
                        ...
                        >
                            <Link href={`/challenges/${entry.slug}`} className="overflow-hidden rounded-3xl border border-line-300 bg-surface-800/60">
                                <Image
