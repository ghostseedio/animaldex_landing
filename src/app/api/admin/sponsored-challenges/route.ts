...
async function loadLookups() {
    const [orgs, venues, rewards, achievements, countries, stats] = await Promise.all([
        rest<Row[]>("business_organizations?select=id,display_name,slug,website_url&order=display_name"),
        rest<Row[]>("campaign_venues?select=campaign_id,display_name,google_place_id,latitude,longitude,validation_radius_m,country_code"),
        rest<Row[]>("campaign_rewards?select=campaign_id,reward_type,achievement_definition_id"),
        ...
        rest<Row[]>("campaign_sponsor_stats_v1?select=campaign_id,participant_count,completed_count,qualifying_capture_count").catch(() => [] as Row[])
    ]);
    return {orgs, venues, rewards, achievements, countries, stats};
}

function assembleDetail(row: Row, lookups: Awaited<ReturnType<typeof loadLookups>>, versions: AdminRuleVersion[]): AdminCampaignDetail {
    const campaignId = asString(row.id);
    const org = lookups.orgs.find((item) => item.id === row.sponsor_organization_id);
    const venue = lookups.venues.find((item) => item.campaign_id === campaignId);
    const reward = lookups.rewards.find((item) => item.campaign_id === campaignId);
    const achievement = reward
        ? lookups.achievements.find((item) => item.id === reward.achievement_definition_id)
        : null;
    ...
    return {
        ...
        minimumCaptureGrade: row.minimum_capture_grade == null ? null : asNumber(row.minimum_capture_grade),
        liveOnly: Boolean(row.live_only),
        externalImportsAllowed: Boolean(row.external_imports_allowed),
        discoveryRadiusM: row.discovery_radius_m == null ? null : asNumber(row.discovery_radius_m),
        geoMode: isGeoMode(asString(row.geo_mode)) ? row.geo_mode as "unrestricted" | "allowlist" | "denylist" : "unrestricted",
        discoveryCountries,
        ...
        stats,
        publishBlockedReason: publishBlockedReason(asString(row.geo_mode) || "unrestricted", hasVenue)
    };
}
...
async function loadDetail(id: string): Promise<AdminCampaignDetail | null> {
    ...
    const [lookups, versions] = await Promise.all([
        loadLookups(),
        rpc<Row[]>("admin_list_sponsored_campaign_rule_versions", {p_campaign_id: id}).catch(() => [] as Row[])
    ]);
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const campaigns = rows.map((row) => {
            ...
            const reward = lookups.rewards.find((item) => item.campaign_id === row.id);
            const achievement = reward
                ? lookups.achievements.find((item) => item.id === reward.achievement_definition_id)
                : null;
            const org = lookups.orgs.find((item) => item.id === row.sponsor_organization_id);
        ...
        return NextResponse.json(assertSafeAdminDto({
            ok: true,
            campaigns,
            organizations,
            appleDisclaimer: APPLE_DISCLAIMER
...
type ActionBody = {
    ...
    rewardTerms?: string;
    organization?: {id?: string | null; displayName: string; slug: string; websiteUrl?: string | null};
    ...
    try {
        if (action === "upsert_organization") {
            ...
            if (!org?.displayName?.trim() || !org.slug?.trim()) {
                return NextResponse.json({ok: false, error: "Organization name and slug are required"}, {status: 400});
            }
        ...
        if (action === "upsert_campaign") {
            if (!body.draft) {
                return NextResponse.json({ok: false, error: "Campaign draft is required"}, {status: 400});
            }
            const row = await rpc<Row>("admin_upsert_sponsored_campaign", serializeCampaignUpsert(body.draft));
        ...
        if (!campaignId) {
            return NextResponse.json({ok: false, error: "A campaign is required"}, {status: 400});
        }

        if (action === "set_venue") {
            if (!body.venue) {
                return NextResponse.json({ok: false, error: "Venue details are required"}, {status: 400});
            }
            await rpc("admin_set_campaign_venue", serializeVenueUpsert(campaignId, body.venue));
        } else if (action === "set_geo") {
            await rpc("admin_set_campaign_geo_countries", serializeGeoCountries(campaignId, body.countryCodes ?? []));
        } else if (action === "set_reward") {
            if (!body.achievement) {
                return NextResponse.json({ok: false, error: "An achievement reward is required"}, {status: 400});
            }
            await rpc("admin_set_campaign_achievement_reward", serializeAchievementReward(campaignId, body.achievement));
        } else if (action === "revise_rules") {
            await rpc(
                "admin_revise_sponsored_campaign_rules",
                serializeRulesRevision(campaignId, body.officialRules ?? "", body.rewardTerms ?? "")
            );
        } else if (action === "submit") {
            await rpc("admin_submit_sponsored_campaign", {p_campaign_id: campaignId});
            ...
            await rpc("admin_archive_sponsored_campaign", {p_campaign_id: campaignId});
        } else {
