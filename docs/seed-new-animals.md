AnimalDex Seed Data: 23 New Indexed Animals

Create new indexed AnimalDex entries for these 23 animals.

Do not insert into species_catalog_v1; it is a view.

For every animal below, create records in:

* species_profiles
* species_subtitles
* species_behavior_principles
* animal_value_profiles

Do not create breed_market_profiles for these entries. Even tosakin-goldfish should be treated as an indexed catalog animal here, so use identity_kind = 'domestic_parent', not breed, and give it an animaldex_number.

Use the next available animaldex_number sequence unless numbers are manually specified elsewhere.

Global defaults

identity_source: refined_identity
identity_confidence: 1.0
generation_status: ready
generation_model_version: seed:manual_catalog
...
Create a Supabase SQL migration or seed script that:

1. Uses the data above as the canonical source.
2. Inserts/updates species_profiles by normalized_identity_key.
3. Assigns consecutive animaldex_number values only when the row is newly inserted or when animaldex_number IS NULL.
4. Does not overwrite existing non-null animaldex_number values.
5. Inserts/updates species_subtitles by species_profile_id.
6. Inserts/updates species_behavior_principles by species_profile_id.
7. Inserts/updates animal_value_profiles by species_profile_id.
8. Does not insert into species_catalog_v1.
9. Does not insert into analysis_results.
10. Does not insert into breed_market_profiles.
11. Sets catalog_status = 'active'.
12. Sets generation_status = 'ready'.
13. Sets catalog_source = 'manual'.
14. Sets generation_model_version = 'seed:manual_catalog'.
...
WITH seed_animals AS (
  SELECT *
  FROM jsonb_to_recordset($$ ... JSON version of the YAML above ... $$::jsonb)
  AS x (
...
numbered AS (
  SELECT
    s.*,
    ROW_NUMBER() OVER (ORDER BY s.landing_page_slug) + existing_max.max_num AS proposed_animaldex_number
  FROM seed_animals s
...
upserted_profiles AS (
  ...
  ON CONFLICT (normalized_identity_key) DO UPDATE SET
    ...
    animaldex_number = COALESCE(species_profiles.animaldex_number, EXCLUDED.animaldex_number)
  RETURNING id, normalized_identity_key, landing_page_slug
)
-- Then use upserted_profiles joined back to seed_animals to upsert:
-- species_subtitles
-- species_behavior_principles
-- animal_value_profiles
COMMIT;
...
* tosakin-goldfish is normally a domestic ornamental variety, but this batch requires 23 new indexed animals, so it should consume an animaldex_number.
* eastern-tiger-swallowtail-caterpillar and eastern-tiger-swallowtail intentionally both use Papilio glaucus, but have different normalized_identity_key values because the caterpillar is a separate indexed life-stage card.
* fruit-bat, hermit-crab, salp, sea-anemone, and sea-urchin are generic parent profiles because the filenames do not identify one exact species.
* spiny-oyster uses Spondylus spp. because the image/file name does not identify a single exact Spondylus species.
* Add optional Swift native-range mappings in NativeRangeCatalog.swift using each landing_page_slug or normalized_identity_key.